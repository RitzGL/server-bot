...
async function weatherFromLocation(forecast, interaction, ephemeral = true) {
  const { id } = interaction.user;

  // check if WW_TOKEN Exists
  if (!process.env.WW_TOKEN) {
    return interaction.reply({
      content: 'No Willy Weather Token found in Environment Variables',
      ephemeral: true,
    });
  }
  ...
  if (!user) {
    return interaction.reply({
      content: 'You havent set a postcode! set with `/setpostcode`',
      ephemeral: true,
    });
  }
  if (!forecast) {
